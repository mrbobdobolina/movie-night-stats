			...
			<div class="row row-cols-1 row-cols-md-2 row-cols-md-2 row-cols-xl-10 g-3 mt-5">
	            ...
	            <div class="card-body">
								<ul>
						     	...
						     	<li><strong>Attendance:</strong> <?php echo $attend;?></li>
									...
									<li><?php $vy = get_viewers_years_single($viewer);
										if(!empty($vy)){
											$vy_count = count($vy);
										} else {
											$vy_count = 1;
										}?>
										Tends to pick movies that were released around <?php echo round(array_sum($vy)/$vy_count); ?></li>
									<li><strong>Scribe: </strong><?php echo countScribe($viewer); ?></li>
									<?php $picks = countMySpins($viewer);?>
									<li><strong>All Picks: </strong><?php echo $picks['total'];?></li>
									<?php $spins = countMySpins_noChoice($viewer); ?>
	          ...
	          <div class="card">
	            ...
	            <div class="card-body">
								...
								?>
								<ul><h3>Unwatched Picks</h3>
									<?php
									foreach($allUserPicks3 as $key => $value):
									?>
									<?php if(!in_array($key, $watchedFilmList)):?>
										<li><?php echo getMovieById($key); ?> <em>(<?php echo $value; ?>)</em></li>
									<?php endif;?>
								<?php endforeach; ?>
								</ul>
	            </div>
